...
class Produto(Base):
    ...
    ean = Column(String, index=True, unique=True)  # Codigo de barras EAN
    data_criacao = Column(DateTime, default=datetime.now)

    precos = relationship("Preco", back_populates="produto")
    alertas = relationship("Alerta", back_populates="produto")
    itens_lista = relationship("ItemLista", back_populates="produto")
...
class ValidacaoAutomatica(Base):
    """Log de validacoes automaticas de precos (auto-moderacao)"""
    __tablename__ = "validacoes_automaticas"
    ...
    preco_submetido = Column(Float, nullable=False)
    mediana_existente = Column(Float)
    diferenca_percentual = Column(Float)
    total_precos_comparados = Column(Integer, default=0)

    limite_aprovacao = Column(Float, default=30.0)
    limite_rejeicao = Column(Float, default=50.0)
    reputacao_alteracao = Column(Integer, default=0)
...
class ListaCompras(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    usuario_nome = Column(String, nullable=False, index=True)
    nome = Column(String, nullable=False, default="Minha Lista")
    data_criacao = Column(DateTime, default=datetime.now)
    ...
    itens = relationship("ItemLista", back_populates="lista", cascade="all, delete-orphan")


class ItemLista(Base):
    """Item em uma lista de compras"""
    ...
    melhor_preco = Column(Float)
    melhor_supermercado = Column(String)
    data_comparacao = Column(DateTime)
